# 07 · 古建保护成果工作台技术架构 v1.4（已确认）

> v1.4 修订范围：第 2 章实现状态更新至 2026-08-16（阶段 A、B 已收口）；4.3 目录边界按当前仓库修正；6.2 本地库 v7 一次纳入五类对象库；第 7 章新增 7.6 助手动作分发；7.3 标注模型参数目标口径；10 核心接口补助手回合服务；14、15、16 章补充完成状态。其余章节与 v1.3 一致。
...
本文件定义目标状态和迁移方案。JIAJIA 已于 2026-08-13 确认 v1.2；截至 2026-08-16，实施任务 T0a 至 T13 已完成（阶段 A、B），剩余实施按第 15 章继续。
...
- 产品范围以 JIAJIA 于 2026-08-13 确认的 [03 核心 PRD](03_核心PRD_古建保护成果工作台.md) v1.1 为准。
- 专业制图和三维模型以 [08 质量基准](08_专业制图与建模质量基准.md) v0.4 为最低标准。
- Kimi、开放制图技术和样本事实以 [02 制图预研](02_正式制图能力与验证样本预研.md) 研究结论及其官方来源为准。
- 人工节点以 [06 人工节点审查](06_AI与人工节点审查.md) 的完整矩阵为准，不得重新加入已删除的确认弹窗。
- 助手动作清单与确认机制以 [05 界面与交互形态](05_界面与交互形态.md) 第八节为准。
- 当前实现状态以本文件第 2 章与里程碑验证材料为准。
- CAD 验收按 PRD v1.1 区分规范资格工具和第二兼容工具。该规则取代 [08 质量基准](08_专业制图与建模质量基准.md) 中第二工具也需编辑、保存和无损往返的旧表述；T0a 同步修订质量基准。

## 2. 当前状态与主要问题

当前仓库已完成阶段 A、B 收口与助手动作层接线，第三项目端到端验证通过；剩余差距集中在规则与词表数据层、图纸质量与私有部署责任（见表 1）。

**表 1：当前实现与目标状态（2026-08-16）**

| 范围 | 当前状态 | 目标状态 | 迁移原则 |
|------|----------|----------|----------|
| 工作台 | 新工作台已建成并通过验收，根入口已切换（T11、T13） | `apps/workbench` 只读取当前项目和查询接口 | 旧入口只用于回归，不继续扩展 |
| 项目数据 | IndexedDB v3 为单一真源，命令权威已统一（T3） | IndexedDB v3 或服务器项目库是单一真源 | 不原位升级 v2；通过迁移器导入 v3 |
| 资料输入 | 原文件、声明、解析与对象关系已进入项目版本（T5） | 原文件、声明、解析、候选和对象关系进入项目版本 | 解析失败保留原文件 |
| AI 助手 | 动作层已接线并通过端到端验收（14 动作、真实分发基准 88.6%，见 7.6）；目录端点与请求口径统一待补 | Node 统一调用 `kimi-k2.6`，保存完整运行事件 | 不显示预置成功结果 |
| 领域内核 | 领域契约、应用命令、IndexedDB 仓库与事务测试已就绪（T3） | 领域契约、应用命令、存储适配器可独立测试 | 现有代码逐项审核后迁入 |
| v2 图纸 | Drawing IR 已从当前 `GeometryRevision` 生成 DXF、SVG、PDF（T9） | Drawing IR 从当前 `GeometryRevision` 生成全部格式 | 保留工程经验，不沿用旧成果资格 |
| v3 几何 | 输入、manifest、源网格、GLB 与验证报告同构建记录（T8b）；图纸并行化后第三项目出图约 156 秒 | 输入、manifest、源网格、GLB 和验证报告使用同一构建记录 | 旧报告作废，整组重新生成 |
| 项目样本 | HABS 成功样本、高都阻断样本与第三项目均完成端到端验证（T9a、T12） | 一套完整 HABS 成功样本、高都阻断样本和第三项目 | 每套资料独立，不混用尺寸或构件 |
| 正式责任 | 固定代理身份和字段占位 | 私有部署接入真实身份、权限和签名 | 本地模式锁定正式签发 |

已完成阶段以里程碑验收证据为准，但只验证工程流程，不代表专业成果或正式资格已经完成，L1 始终保持 false。
...
自动应用条件以 [03 核心 PRD](03_核心PRD_古建保护成果工作台.md) 第 2.3 节为准。每次判断建立 `PolicyEvaluation`；未通过时生成补测问题、非唯一方案或成组预览，不增加逐项确认弹窗。
...
助手动作分发把 [05 界面与交互形态](05_界面与交互形态.md) 第八节动作清单映射为受控执行。当前目录 14 个动作；服务端负责调度、校验与留痕，写项目数据的动作由客户端经唯一命令入口执行。
...
兼容副本必须使用不同文件名和清单状态。后续替换兼容工具时，由 R001 和发布测试矩阵决定，不修改领域模型。
...
人工节点与确认口径以 [03 核心 PRD](03_核心PRD_古建保护成果工作台.md) 第 2.3 节为准：只在补充现场事实、选择非唯一专业方案、成组审核与签发三类节点等待人工，其余不设置固定确认。
...
截至 2026-08-16：HABS 成功样本与高都阻断样本均完成端到端验收；第三项目完成空库导入、几何生成、图纸生成、导出与回导验证。验收证据在验证材料目录。
...
T0a 已将 `.claude/context/pm-context.md` 更新为 PRD v1.1、架构 v1.2、当前分支和审查报告状态，并将质量基准更新为资格工具与兼容工具分工。后续角色不得继续引用旧分支、旧 T0-B 状态或第二 CAD 无损保存规则。

**表 14：实施任务**

| 任务 | 输出 | 依赖 | 提交建议 | 验证与回退 |
|------|------|------|----------|------------|
| T0a 更新权威依据 | PRD v1.1、架构 v1.2、当前分支和审查报告状态写入 `pm-context`；质量基准改为资格/兼容双工具规则 | 架构确认 | `docs(context)` | 状态和 CAD 规则无冲突；撤销该提交 |
...
截至 2026-08-16，T0a 至 T13 已完成（阶段 A、B）；T14、T15 与 R1 至 R4 待开始。
...
JIAJIA 已确认 v1.2 与 v1.4。T0a 至 T13 与动作层接线已完成；剩余实施从 R1 至 R4 数据层与 C、D 收口开始，顺序经 JIAJIA 确认后执行。
