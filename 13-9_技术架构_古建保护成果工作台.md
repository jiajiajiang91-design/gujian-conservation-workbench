...
自动应用条件以 13-8 第 2.3 节为准。每次判断建立 `PolicyEvaluation`；未通过时生成补测问题、非唯一方案或成组预览，不增加逐项确认弹窗。
...
助手动作分发把 13-6 第八节动作清单映射为受控执行。当前目录 14 个动作；服务端负责调度、校验与留痕，写项目数据的动作由客户端经唯一命令入口执行。
...
人工节点与确认口径以 13-8 第 2.3 节为准：只在补充现场事实、选择非唯一专业方案、成组审核与签发三类节点等待人工，其余不设置固定确认。
