# 13-9 · 古建保护成果工作台技术架构 v1.3（草案待确认）

> v1.3 修订范围（对应 PRD v1.2 的 R012 至 R014）：新增 5.5 规则集与求值器、5.6 受控词表、5.7 形制参数层；6.2 本地库升级 v7 增加两类对象库；6.3 项目包纳入词表与形制记录；10 核心接口补多方案选择结构；15 实施任务追加 R1 至 R4。其余章节与 v1.2 一致。

## 1. 架构结论

目标系统采用“React 工作台 + TypeScript 领域内核 + 独立 Node 服务 + Python 制图 worker”。项目数据只有一个写入入口。AI、规则和制图程序都只能返回候选或成果，不能直接改写项目。

本地模式以浏览器 IndexedDB v3 为项目真源。私有部署以服务器数据库和对象存储为项目真源。两种模式共用领域契约，但同一部署只能启用一个命令权威。
...
本文件定义目标状态和迁移方案，不表示当前代码已经实现这些能力。JIAJIA 已于 2026-08-13 确认 v1.2；实施仍按任务门槛逐项开始。

### 1.1 架构目标

- R001 至 R011 都能映射到数据、模块、接口和测试。
...
- 产品范围以 JIAJIA 于 2026-08-13 确认的 `13-8` 核心 PRD v1.1 为准。
- 专业制图和三维模型以 `13-12` 质量基准 v0.4 为最低标准。
- Kimi、开放制图技术和样本事实以 `13-11` 研究结论及其官方来源为准。
- 人工节点以 `13-14` 的完整矩阵为准，不得重新加入已删除的确认弹窗。
- 当前实现状态与未提交改动以 `13-13` 审查报告和实施前文件清单为准。
- CAD 验收按 PRD v1.1 区分规范资格工具和第二兼容工具。该规则取代 `13-12` 中第二工具也需编辑、保存和无损往返的旧表述；T0a 同步修订质量基准。

## 2. 当前状态与主要问题

当前仓库包含旧工作台、根级领域内核、v2 图纸试验和 v3 几何试验，但尚未形成完整生产流程（见表 1）。

**表 1：当前实现与目标状态**

| 范围 | 当前状态 | 目标状态 | 迁移原则 |
|------|----------|----------|----------|
| 工作台 | 旧 React 页面直接导入高都、东呈，并按固定 UUID 选择生成器 | `apps/workbench` 只读取当前项目和查询接口 | 旧入口只用于回归，不继续扩展 |
| 项目数据 | IndexedDB v2、旧 JSON 和部分命令实验并存 | IndexedDB v3 或服务器项目库是单一真源 | 不原位升级 v2；通过迁移器导入 v3 |
| 资料输入 | 资料页以展示为主 | 原文件、声明、解析、候选和对象关系进入项目版本 | 解析失败保留原文件 |
| AI 助手 | 页面返回本地固定建议；部分任务仍使用 `moonshot-v1-128k` | Node 统一调用 `kimi-k2.6`，保存完整运行事件 | 不显示预置成功结果 |
| 领域内核 | 已有项目、事实和命令基础；基础设施包没有源码 | 领域契约、应用命令、存储适配器可独立测试 | 现有代码逐项审核后迁入 |
| v2 图纸 | 有 DXF、SVG、PDF 和字体试验；与 v3 几何不同源 | Drawing IR 从当前 `GeometryRevision` 生成全部格式 | 保留工程经验，不沿用旧成果资格 |
| v3 几何 | 合同、生成器和验证器有可复用内容；当前报告与输出哈希不一致 | 输入、manifest、源网格、GLB 和验证报告使用同一构建记录 | 旧报告作废，整组重新生成 |
| 项目样本 | Dai Loy 只有精简子集；高都、东呈是演示数据 | 一套完整 HABS 成功样本、高都阻断样本和第三项目 | 每套资料独立，不混用尺寸或构件 |
| 正式责任 | 固定代理身份和字段占位 | 私有部署接入真实身份、权限和签名 | 本地模式锁定正式签发 |

现有测试和生成文件只能证明对应技术试验。它们不能证明专业成果、成功样本或正式资格已经完成。
...
|------|----------|----------|----------|----------|
| 本地验证 | 浏览器内 `ProjectCommandService` | IndexedDB v3 | IndexedDB Blob | 锁定，只允许代理交付 |
...
├─ adapters/           项目包、模型、规则、成果和身份适配
└─ infrastructure/     IndexedDB、服务器仓库、Blob 和进程通信
workers/
...
新代码不写入 `交互原型/02_AI工作台`。旧目录只供迁移测试和行为回归读取。样本保存在 `验证材料` 或测试资源目录，通过项目包加载。
...
生成器不能写入 L1 通过结论。独立验证器只能报告确定性检查结果。没有独立专业审核时，成果保持 `generated-not-qualified` 和 `L1=false`。

模型转写的尺寸继续保留 `model` 来源。缺少测量人、时间、方法、单位或原始记录时，对应 `FactEligibilityEvaluation` 必须返回阻断码，尺寸不得用于正式成果、正式标注或 L1 资格。人工确认转写准确不能解除这些阻断。
...
规则是数据文件，不是提示词，也不是散落在程序里的分支。每条规则专业人员逐条可审，并标注规范出处。
...
- 同一输入可按多个并存规则集分别计算，输出并列方案供规范选择停靠（10 章）。规范差异表达为并存规则集，不是互斥真理。
...
同一责任人同时具备审核与签发权限时，`CommitProfessionalReview` 在一次用户操作中原子建立 `ProfessionalReview`、`ArtifactApprovalSignature` 和受控归档签名授权。

签名覆盖不可变 `artifactSetHash`、`professionalReviewHash`、项目版本、签发人、资格条件、策略版本和有效期。签名服务失败时，审核与签发均不提交为可见记录，不能用“签名请求”代替签名。

组织制度要求职责分离时，`CommitProfessionalReview` 只建立审核记录。第二名有权人员执行一次 `SignDeliveryDraft`，核对未变化的成果集、审核、资格和清单后建立成果批准签名和交付清单签名，不重复填写或执行专业审核。所有关键记录都通过当前部署命令权威建立。

### 6.2 本地 v3 数据库

新数据库名为 `gujian-workbench-v3`。初始数据库版本为 1。主要对象库包括项目、版本、资源、运行、候选、规则、决定、几何、成果、检查、交付、审计、导入会话和命令回执。

v1.3 起数据库版本升级为 7，新增 `conceptEntries`（词表条目）与 `archetypeSpecs`（形制参数）两个对象库，一次升级完成。升级期间其它标签页占用旧版本会阻塞升级，界面提示用户关闭其它页面后重试。
...
首期任务类型包括任务提取、资料分类、测量转写、对象候选、问题分析和审核摘要。各任务共用事实责任边界，但使用独立输入与输出 schema。
...
模型候选只有同时满足以下条件才可自动应用：

- 证据直接支持，或受信规则只有一个结果。
- 操作不新增现场事实，不改变规范、责任人、专业方案或已签发成果。
- 操作可撤销，影响范围可计算，确定性检查通过。
- 项目策略允许该任务和操作。
- 当前模型任务组合通过固定测试集，且资格记录仍有效。

每次判断建立 `PolicyEvaluation`。未通过时生成补测问题、非唯一方案或成组预览，不增加逐项确认弹窗。
...
`ViewDefinition` 来自 R001，定义视图类型、方向、剖切面、范围、比例和可见规则。平面、立面和剖面不得另用参数重新画一套建筑。
...
成果目录和布局由 `TaskDefinition` 与 `ArtifactRequirementMatrix` 驱动。视图数、图幅数、每张布局的视口数、标题、图号、日期和责任边界都来自当前任务与版面计算。生成器不得固定 10 个视图、2 张 A1、每页 5 个视口、内部标题 ID 或测试日期。

同一 `DrawingIR` 生成原生 DXF、SVG、PDF 和 300 dpi 检查图。各格式保存相同的 `geometryRevisionId`、视图 ID 和来源 sidecar。

### 8.3 成果和阻断

R005 的每个图种必须在 `ArtifactRequirementMatrix` 中标为：

- `required/generated`：已经生成真实文件并通过程序检查；
- `required/blocked`：资料不足，并保存阻断码；
- `notRequired`：R001 明确不需要。

空图、空图框、默认场地、通用构件和未关联来源的补画不能标为 `generated`。资料不足时生成可支持的局部成果、未知区域和补测要求。

### 8.4 CAD 资格与兼容

规范 DXF 使用 R2018、模型空间 1:1 mm、原生 `DIMENSION`、`TEXT/MTEXT`、`HATCH`、`INSERT`、`LAYOUT` 和锁定 `VIEWPORT`。项目图层、字体、线型、线宽、图幅、比例和打印设置来自 R001。
...
QCAD 保存会改写原生尺寸和视口锁定，因此兼容副本必须使用不同文件名和清单状态。后续替换兼容工具时，由 R001 和发布测试矩阵决定，不修改领域模型。
...
系统只在三类责任上等待人工：补充缺失现场事实；选择没有唯一答案的专业方案；审核成组成果并承担签发责任。

资料分类、对象关联、测量转写、唯一规则结果、成果目录、比例、图幅、技术检查、资格计算和流程状态不设置固定确认。驳回、改写或责任制度要求时才输入理由。
...
组织职责分离时，步骤 2 只建立 `ProfessionalReview`，不建立成果批准签名或归档签名授权。系统建立待签 `DeliveryDraft` 后，由第二名有权人员执行一次 `SignDeliveryDraft`。

该命令同时建立 `ArtifactApprovalSignature` 和 `DeliveryManifestSignature`，随后执行验签和正式资格评估。第二人核对已有审核与阻断项，不重复审核未变化的成果。
...
现有未提交改动必须逐项审核并纳入迁移判断，不能绕过脏工作区建立一套无关实现。

### 12.1 现有改动处置

Git 标签不能保存未提交和未跟踪文件。任何分类、移动或清理之前，先建立可实际恢复的本地快照：

1. 在忽略目录 `归档/recovery/2026-08-13-pre-rebuild/` 保存当前分支、HEAD、Git 状态和工具版本。
...
| 根领域和应用契约 | 修正后迁移 | 项目、事实、命令和来源基础可复用 | 与 PRD v1.1、v3 schema、事务和迁移测试一致 |
...
上一轮目标任务产生的源码、合同、测试、字体和报告都先进入资产清单。未经文件级审查，不删除、不覆盖、不重写已经验证的通用能力。分类为“修正后迁移”的资产采用最小改动，并保留原测试；分类为“失败或历史证据保留”的资产保留失败原因、绑定版本和哈希，但不进入当前运行路径。

表中的“淘汰”只表示退出目标运行路径、Git 跟踪或生成依赖，不授权删除用户原文件。删除本地外部参照或历史资产需要单独确认。

删除状态的 AutoCAD、QCAD 证据必须先记录其绑定的旧 DXF 文件哈希、来源提交、失效原因和替代报告，再决定恢复或保留删除状态。不得把删除本身当作证据处置完成。

每个处置任务先记录文件清单和 SHA-256。有效源码、测试、许可和小型审查报告进入对应任务提交；可重复生成的大文件按 §12.4 处理。
...
实施分支为 `codex/professional-workbench-rebuild`。本地 HEAD 与 `origin/main` 当前没有共同祖先，发布必须采用非破坏整合，禁止 force-push。

远端为 `jiajiajiang91-design/gujian-conservation-workbench`。整合顺序固定如下：

1. 获取并核对 `origin/main` 的当前提交 `ac6abbc`。先建立远端只读备份分支 `archive/portfolio-v0.0` 和标签 `portfolio/v0.0`，两者都精确指向该提交。
2. 为拟发布的精确提交集合建立提交清单和文件清单。逐项扫描密钥、用户名、绝对路径、私有资料、许可、外部 DWG、大对象和过期资格表述。
3. 从 `origin/main` 建立 `codex/integration-professional-workbench`。只导入通过审查的本地任务提交；不能安全导入的历史使用净化快照，并保留来源提交映射。
4. 只有本地完整历史也通过同等扫描时，才允许使用 `--allow-unrelated-histories` 建立合并候选。任一历史风险未解决时，使用 `cherry-pick` 或净化快照，不连接两套完整历史。
5. 在 integration 分支运行构建、测试、许可和安全检查，通过 PR 合并到 `main`。不得直接覆盖或强制更新 `main`。
6. 发布回退通过 revert PR 或 revert 合并提交完成。远端归档分支、标签和现有公开提交保持不变。

每个实施任务只包含一个可验证结果，使用独立提交。功能回退使用 `git revert`，不得通过删除 v2、v3 或用户项目完成。
...
R001 至 R011 的技术责任见表 11。

**表 11：需求、数据、模块与验收映射**

| 需求 | 主要数据 | 主要模块 | 核心接口 | 验收重点 |
|------|----------|----------|----------|----------|
| R001 | `TaskDefinition`、角色、自动策略 | 任务、身份、影响 | 命令、查询、决定 | 成果要求唯一来源；变化精确失效 |
| R002 | `Asset`、`Evidence`、解析记录 | 入库、文件解析、Kimi | 入库、模型网关 | 原文件保留；失败可恢复 |
| R003 | 对象、测量、事实资格、未知项、几何版本 | 领域、资格、几何 worker | 资格、几何、命令 | 不合格尺寸禁入正式成果；不补默认值 |
| R004 | 问题、候选、规则、策略和决定 | Kimi、规则、决定 | 模型、规则、决定 | AI 先处理；只审核非唯一方案 |
| R005 | 要求矩阵、视图、Drawing IR、成果 | 几何、制图、检查 | 几何、制图、成果 | 全图种生成或阻断；无空图 |
| R006 | 事实资格、检查、专业审核、批准签名、成果资格和责任 | 检查、授权、签名、交付 | 检查、资格、身份、签名 | 同人一次审核签发；职责分离才二次操作 |
| R007 | 交付、接收、退回、版本和完整外部事件前缀 | 交付、影响、归档 | 交付、审计 | 同步完成后交付；局部退回 |
| R008 | 清单、项目、资源、审计 | 项目包、暂存、仓库 | 项目包、命令 | 原子导入、导出和完整回导 |
| R009 | 页面状态、来源图、运行状态 | React 工作台、助手 | 查询、命令、SSE | 1280 px、键盘和失败降级 |
| R010 | 运行、事件、用量、费用和资格 | 任务注册表、模型服务 | 模型网关、运行查询 | 流式、取消、迟到和真实费用 |
| R011 | 组织、成员、权限和责任快照 | 身份、授权、签名 | 身份、权限、交付 | 本地锁定；私有部署实际验证 |
...
成功样本先审核 Badin-Roque HABS 全套资料。若最低资料项不齐，改用完整 Dai Loy 或下一套满足条件的 HABS 档案。精简 Dai Loy 子集只能做局部验证，不能计入成功路径。
...
| 层级 | 必须覆盖 |
|------|----------|
| 旧行为 | 现有 13 项服务回归和 17 项应用单元测试；固定项目反例 |
| 数据单元 | schema、来源、单位、未知项、依赖、资格和精确失效 |
| 命令集成 | 两种权威、输入快照、幂等事件同步、版本冲突、事务回滚和迟到结果 |
| 项目包契约 | JSON/ZIP 往返、外部事件前缀、哈希、MIME、远程资源、路径穿越和两阶段导入 |
| Kimi 契约 | `kimi-k2.6`、固定输入包、loopback 安全、流式、工具、预算、取消、重试、迟到和用量 |
| Kimi 在线 | 使用服务端密钥完成最小成功、失败和取消冒烟；密钥缺失时明确跳过 |
| 几何 | 固定输入包、事件同步、单位、未知项、对象映射、真实界面、剖切、投影和无固定形制 |
| 文件 | IFC、GLB、DXF、SVG、PDF、报告和预览解析；`geometryRevisionId` 与哈希关系完整 |
| CAD | 两种成果目录与布局；AutoCAD 规范资格；QCAD 打开、查看和打印兼容 |
| 端到端 | HABS 成功、高都阻断、第三项目、模型失败、任务恢复和项目包回导 |
| 浏览器 | 1280 px、1440 px、键盘、焦点、长表格、三维和助手收起 |
| 专业审核 | 成组三维和图纸预览；P0 为 0；资格仍由独立记录计算 |
| 审核签发 | 同一责任人一次操作生成审核与成果批准签名；职责分离时第二人一次签发；授权变化失效 |
...
T0a 已将 `.claude/context/pm-context.md` 更新为 PRD v1.1、架构 v1.2、当前分支和 `13-13` 状态，并将 `13-12` 更新为资格工具与兼容工具分工。后续角色不得继续引用旧分支、旧 T0-B 状态或第二 CAD 无损保存规则。

**表 14：实施任务**

| 任务 | 输出 | 依赖 | 提交建议 | 验证与回退 |
|------|------|------|----------|------------|
| T0a 更新权威依据 | PRD v1.1、架构 v1.2、当前分支和 `13-13` 状态写入 `pm-context`；`13-12` 改为资格/兼容双工具规则 | 架构确认 | `docs(context)` | 状态和 CAD 规则无冲突；撤销该提交 |
...
| T14 完成私有责任适配 | 服务器仓库、身份、权限和签名 | T13 | `feat(private)` | R011 和正式签发测试 |
| T15 非破坏发布求职仓库 | 远端备份引用、净化 integration 分支、README、许可、CI 和 PR | T13 | `docs(showcase)` | 精确提交扫描、无旧大对象、禁止 force-push；revert PR 回退 |
| R1 声明式规则层 | 规则文件 schema、第一批规则数据、求值器和 workflow 接入 | 架构 v1.3 确认 | `feat(rules)` | 求值器单测（公式、依赖、按实计、双规则集）；规则文件全表校验 |
| R2 多方案停靠 | Issue 方案数组、Decision 所选方案、问题队列方案选择界面 | R1 | `feat(rules)` | 旧记录兼容；浏览器完成一次带出处的方案选择 |
| R3 受控词表 | ConceptEntry、种子词表、IndexedDB v7、双写与闭包匹配、项目包往返 | 架构 v1.3 确认 | `feat(vocabulary)` | 词表校验、旧包兼容导入、含词表包空库往返 |
...
JIAJIA 已确认 v1.2。项目先执行 T0a，再执行 T0b；T0b 完成后暂停，等待资产分类与提交拆分确认，不自动进入 T1。
