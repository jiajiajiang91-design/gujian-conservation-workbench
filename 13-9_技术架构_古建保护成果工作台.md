# 13-9 · 古建保护成果工作台技术架构 v1.1（质量基准已确认）
...
- 专业制图和三维模型以 `13-12_专业制图与建模质量基准.md` 为最低验收标准。
...
T0 分为两个连续门槛。T0-A 使用最小几何样例验证文件链路、稳定对象映射和外部软件兼容性；T0-B 使用古建局部专业样板验证曲面屋面、瓦作、木构、门窗、台基、同源剖切和专业图面。T0-A 通过不能替代 T0-B，也不能作为专业成品展示。

T0 根据 IfcOpenShell 官方支持矩阵选择 Python 和依赖版本，再生成带哈希的锁文件。锁文件记录 wheel 来源与哈希、字体包、PDF 后端、线型和模板。架构不预先臆定版本。只有 T0-A 证明 IfcOpenShell 随附能力不能满足稳定对象映射、剖切或隐藏线时，才在锁文件中增加独立 OpenCascade 绑定。
...
T10 的三维、图纸和 CAD 同时执行 `13-12_专业制图与建模质量基准.md`。体块模型、平板屋面、空白门窗、无真实构造的剖面和不可编辑的矢量外观不构成专业成果。
...
T0 是领域建设前的技术可行性与专业表现任务。T0-A 必须在目标 Windows 设备验证冷启动、安装和卸载；稳定 `entityId`；IFC、GLB、平面、立面、剖面、DXF、SVG 和 PDF 生成与反向解析；作业取消、进程退出和临时目录清理。T0-B 必须按 `13-12_专业制图与建模质量基准.md` 完成古建局部专业样板。Python、依赖、wheel 来源、字体和 PDF 后端在验证后写入带哈希锁文件。T0-A 与 T0-B 都通过后，T0 才通过。

| 任务 | 输入 | 输出 | 依赖 | 验证与回退 |
|------|------|------|------|------------|
| T0 CAD 可行性与质量验证 | 官方支持矩阵、最小几何样本和古建局部专业样板 | 技术结论、质量结论、带哈希锁文件和可复现报告 | 无 | T0-A 链路与 T0-B 专业样板都通过；撤销 T0 |
...
| T10 生成 R005 全图种 | 几何版本、图纸规范、质量基准和要求矩阵 | 三维、总平、层与屋顶平面、立剖、构件详图、门窗/材料/病害表、DXF/SVG/PDF | T9 | 逐项成果或阻断、专业图面、文件与跨视图测试；撤销 T10 |
...
| T14 三基准验收 | 两套完整工程包、南禅寺局部包和质量基准 | R005 成果矩阵、双完整同路径和局部阻断报告 | T13 | 达到 L2、无空图、端到端与专业复核；根入口不切换 |
...
JIAJIA 已于 2026-08-11 确认方案 B、Python 制图 worker 和本地模式正式签发锁定。实施从 T0 开始，后续任务不得绕过前置验收。
