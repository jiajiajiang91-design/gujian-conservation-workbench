# 13-9 · 古建保护成果工作台技术架构 v1.2（待确认；v1.1 已确认）
...
本文件定义目标状态和迁移方案，不表示当前代码已经实现这些能力。JIAJIA 确认 v1.2 后，实施任务才可以开始。
...
当前 `.claude/context/pm-context.md` 仍记录 PRD v1.0、架构 v1.1、旧分支和旧 T0-B 状态。`13-12` 也保留了第二 CAD 需要无损保存的旧规则。v1.2 确认后，必须先更新这两份权威依据，再开展迁移或代码任务。
...
JIAJIA 确认 v1.2 后，项目先执行 T0a，再执行 T0b。架构确认前不修改应用、领域、制图或样本代码。
