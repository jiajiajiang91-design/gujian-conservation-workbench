...
古建保护成果工作台面向承担测绘与保护成果生产的团队，把一栋建筑的照片、实测记录、历史图纸和任务要求，转成可追溯的三维模型、专业图纸和交付档案。AI 负责理解资料、分析问题和推进流程，确定性程序负责几何计算与格式生成，专业人员负责专业判断与责任签发。产品交付的不是一次性数据，而是可复查、可更新、可长期使用的建筑档案。
...
为承担古建测绘与保护成果生产的团队，解决建筑档案建不完、建不规范、建完用不起来的问题，让每一栋建筑都拥有一份可追溯、可更新、可长期使用的数字档案。
...
完整调研见 `03_开源agent技术借鉴.md` 与 `04_开源代码借鉴与现状评估.md`。
...
| 版本 | 目标 | 范围 | 不做 |
|---|---|---|---|
| v1.0 | 从资料到代理成果的生产闭环 | 单机工作台，本地数据，代理交付 | 法定签发、多人在线协作 |
...
| F18 | 助手动作执行 | 横向 | 全部页面 | P0 | 本期交付 14 个受控动作，确认级别与留痕见界面文档第八节 |

页面定义见 `07_界面与交互形态.md` 表 2。
...
- 演示：产品对外展示使用的项目、选择依据与演示路径见 `10_演示项目定义.md`，演示数据复用验证样本，不另造数据。
- 判定依据：人工节点的完整矩阵见 `08_AI与人工节点审查.md`，助手动作清单与确认机制见 `07_界面与交互形态.md` 第八节，专业成果质量底线见 `../02_技术/02_专业制图与建模质量基准.md`。
