...
> v1.6 修订范围：全文所引 PRD 版本统一为 v2.0；修正两处失效章节引用；9.1 工作台结构对齐界面文档十视图；T0a 段落标为历史记录。
> v1.5 修订范围：第 13 章需求映射改为 PRD v2.0 的模块编号 M1 至 M5；正文中的旧需求编号引用同步替换。
...
- 产品范围以 [核心 PRD](../01_产品/05_核心PRD.md) v2.0 为准。
- 专业制图和三维模型以 [质量基准](02_专业制图与建模质量基准.md) v0.4 为最低标准。
- Kimi、开放制图技术和样本事实以 [制图预研](../01_产品/02_正式制图能力与验证样本预研.md) 研究结论及其官方来源为准。
- 人工节点以 [人工节点审查](../01_产品/08_AI与人工节点审查.md) 的完整矩阵为准，不得重新加入已删除的确认弹窗。
- 助手动作清单与确认机制以 [界面与交互形态](../01_产品/07_界面与交互形态.md) 第八节为准。
- 当前实现状态以本文件第 2 章与验证证据目录为准。
- CAD 验收按 PRD 区分规范资格工具和第二兼容工具。该规则取代 [质量基准](02_专业制图与建模质量基准.md) 中第二工具也需编辑、保存和无损往返的旧表述；T0a 同步修订质量基准。
...
自动应用条件以 [人工节点审查](../01_产品/08_AI与人工节点审查.md) 第 3 节为准。每次判断建立 `PolicyEvaluation`；未通过时生成补测问题、非唯一方案或成组预览，不增加逐项确认弹窗。
...
助手动作分发把 [界面与交互形态](../01_产品/07_界面与交互形态.md) 第八节动作清单映射为受控执行。当前目录 14 个动作；服务端负责调度、校验与留痕，写项目数据的动作由客户端经唯一命令入口执行。
...
人工节点与确认口径以 [核心 PRD](../01_产品/05_核心PRD.md) 第 3.1 节与 [人工节点审查](../01_产品/08_AI与人工节点审查.md) 为准：只在补充现场事实、选择非唯一专业方案、成组审核与签发三类节点等待人工，其余不设置固定确认。
...
- 中央区域按 [界面与交互形态](../01_产品/07_界面与交互形态.md) 表 2 的十个工作区视图切换；项目列表与模型运行费用为表 3 的项目级页面，不随建筑任务切换。
...
| 根领域和应用契约 | 修正后迁移 | 项目、事实、命令和来源基础可复用 | 与 PRD v2.0、v3 schema、事务和迁移测试一致 |
...
PRD v2.0 的五个模块与横向支撑对应的技术责任见表 11。模块编号 M1 至 M5 取代旧版 R001 至 R014 编号体系；功能编号 F01 至 F18 见 PRD 第 4.3 节。
...
助手动作层的实现见 §7.6，其动作清单与确认机制以 `文档/01_产品/07_界面与交互形态.md` 第八节为准。
...
截至 2026-08-16：HABS 成功样本与高都阻断样本均完成端到端验收；第三项目完成空库导入、几何生成、图纸生成、导出与回导验证。验收证据在 `文档/05_验证证据` 目录。
