...
| 项目数据 | 本地 v3 库（数据库版本 7）为单一真源，命令权威已统一（T3） | 本地 v3 库或服务器项目库是单一真源 | 不原位升级 v2；通过迁移器导入 v3 |
| 资料输入 | 原文件、声明、解析与对象关系已进入项目版本（T5） | 原文件、声明、解析、候选和对象关系进入项目版本 | 解析失败保留原文件 |
| AI 助手 | 动作层已接线并通过端到端验收（14 动作已定义并接线、13 个端到端可执行，正式用例集分发命中率 95.5%，见 7.6）；目录端点与请求口径统一待补 | Node 统一调用 `kimi-k2.6`，保存完整运行事件 | 不显示预置成功结果 |
...
助手动作层的实现见 §7.6，其动作清单与确认机制以 `文档/01_产品/03_界面与交互形态.md` 第八节为准。
