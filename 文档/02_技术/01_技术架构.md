...
> v1.8 修订范围：同步 R1 至 R4 与阶段 C 差距清单的完成状态；表 1 工作台行补界面重构进展。
> v1.7 修订范围：4.3 目录边界改为实际结构并说明测试就近放置；14.2 表 13 增加执行方式列，区分自动化与手工验证。
...
本文件定义目标状态和迁移方案。JIAJIA 已于 2026-08-13 确认 v1.2；截至 2026-08-17，实施任务 T0a 至 T13（阶段 A、B）与 R1 至 R4 已完成，阶段 C 的差距清单已产出，剩余实施按第 15 章继续。
...
- 产品范围以 [核心 PRD](../01_产品/03_核心PRD.md) v2.0 为准。
- 专业制图和三维模型以 [质量基准](02_专业制图与建模质量基准.md) v0.4 为最低标准。
- Kimi、开放制图技术和样本事实以 [AI 与制图能力边界](../01_产品/01_AI与制图能力边界.md) 研究结论及其官方来源为准。
- 人工节点以 [人工节点审查](../01_产品/06_AI与人工节点审查.md) 的完整矩阵为准，不得重新加入已删除的确认弹窗。
- 助手动作清单与确认机制以 [界面与交互形态](../01_产品/05_界面与交互形态.md) 第八节为准。
...
| 范围 | 当前状态 | 目标状态 | 迁移原则 |
|------|----------|----------|----------|
| 工作台 | 已建成并通过验收，根入口已切换（T11、T13）；按交互规范完成三栏与数据证据双半区重构，补齐现状记录与图纸样式两视图，界面文字按书写规范清理 | `apps/workbench` 只读取当前项目和查询接口 | 旧入口只用于回归，不继续扩展 |
...
自动应用条件以 [人工节点审查](../01_产品/06_AI与人工节点审查.md) 第 3 节为准。每次判断建立 `PolicyEvaluation`；未通过时生成补测问题、非唯一方案或成组预览，不增加逐项确认弹窗。
...
助手动作分发把 [界面与交互形态](../01_产品/05_界面与交互形态.md) 第八节动作清单映射为受控执行。当前目录 14 个动作；服务端负责调度、校验与留痕，写项目数据的动作由客户端经唯一命令入口执行。
...
人工节点与确认口径以 [核心 PRD](../01_产品/03_核心PRD.md) 第 3.1 节与 [人工节点审查](../01_产品/06_AI与人工节点审查.md) 为准：只在补充现场事实、选择非唯一专业方案、成组审核与签发三类节点等待人工，其余不设置固定确认。
...
- 中央区域按 [界面与交互形态](../01_产品/05_界面与交互形态.md) 表 2 的十个工作区视图切换；项目列表与模型运行费用为表 3 的项目级页面，不随建筑任务切换。
...
助手动作层的实现见 §7.6，其动作清单与确认机制以 `文档/01_产品/05_界面与交互形态.md` 第八节为准。
...
JIAJIA 已确认 v1.2 与 v1.4。T0a 至 T13、动作层接线与 R1 至 R4 数据层已完成，阶段 C 的图纸差距清单已产出。剩余实施为标注体系补齐（待 JIAJIA 决定投入）、阶段 D 收口、T14 与 T15，顺序经 JIAJIA 确认后执行。
