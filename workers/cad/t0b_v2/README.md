...
独立 `DrawingPackageContract` 已绑定几何版本、视图合同版本、十份 ViewGeometry、双 A1 布局和十二项制图门槛。`DrawingPackageIR` 只复制已验证的结构线与材料区，保存可逆 staging 变换、标注要求和来源 sidecar，不补画或重算建筑几何。独立验收状态为 `passed-contract-and-ir-only`。当前仍没有 DXF、SVG、PDF 或整图 PNG；许可字体未绑定，承托详图仍是简化团队样板，两项继续阻断 L1。

原生 `T0B.dxf` 已由同一 IR 生成并通过独立反向解析。文件为 R2018，模型空间 1:1 mm，包含两个 A1 布局、十个锁定视口、原生尺寸、填充、块、文字、结构 XDATA 和来源 sidecar。AutoCAD 2024 Core Console 对隔离副本执行 AUDIT，结果为 0 错误、0 修复、0 删除。该结果只接受 `native-dxf-only`；第二 CAD 往返、许可字体、SVG、PDF、整图 PNG 和专业复核仍未完成。
...
当前目录已完成第 1、2 步、视图合同、十张中间 ViewGeometry、制图包合同、中间 IR 和原生 DXF。生成记录固定为 `generated-not-qualified`；第二 CAD 往返、SVG、PDF、许可字体、成组图纸和专业复核全部通过前不得申请 L1。
...
受控局部样本使用 trimesh 和 Shapely 完成真实剖切、剖后投影，以及同源轮廓边、特征边、构件边界和解析遮挡。下一步绑定许可字体，由同一 IR 生成 SVG、PDF 和 A1 整图，并完成 QCAD 往返与成组复核。T10 / L2 继续保留 IfcOpenShell / OpenCascade HLR 路线。
