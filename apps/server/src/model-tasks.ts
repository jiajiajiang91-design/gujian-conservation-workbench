...
export interface ModelTaskDefinition {
  ...
  readonly inputKind: ModelTaskInputKind;
...
export const MODEL_TASKS: readonly ModelTaskDefinition[] = [
  ...
  },
  {
    ...
    systemPrompt: [
      ...
      "任务是把实测图纸上已经标注的尺寸读出来，逐条给出图上原文、对应部位和所在位置。",
      "只转写图面上写明的数字，图上没有标注的尺寸一律不给，不按比例量取也不按经验推算。",
      ...
      "evidenceRef 取自哪份资料的标识、locationZh 在图上的位置描述或 null、certainty 取 certain 或 uncertain、noteZh 备注或 null）、",
